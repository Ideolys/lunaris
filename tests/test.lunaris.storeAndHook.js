...
describe('lunaris store', function () {
  ...
  describe('transaction', () => {
    ...
    it.skip('should rollback a store : INSERT', done => {
      lunarisGlobal._stores['multiple1'] = initStore('multiple1'); // multiple1 must crash
      lunarisGlobal._stores['store1']    = initStore('store1');

      var _events = [];
      lunarisGlobal.hook('insert@multiple1', () => {
        _events.push('insert@multiple1');
      });
      lunarisGlobal.hook('inserted@multiple1', () => {
      ...
      lunarisGlobal.insert('@multiple1', [
        { id : 1, label : 'A' },
        { id : 2, label : 'B' }
      ]);
      lunarisGlobal.commit(() => {
        should(_events).eql([
          ...
          'inserted@store1',
          'insert@multiple1',
          'delete@store1', // Remember, lunarisGlobal.delete sends two delete events, onve before HTTP request and one after
          'delete@store1', // Remember, lunarisGlobal.delete sends two delete events, onve before HTTP request and one after
          'deleted@store1',
        ]);
        done();
    ...
    it.skip('should rollback a store : UPDATE', done => {
      ...
      lunarisGlobal.hook('inserted@store1', (insertedObjs) => {
        ...
        lunarisGlobal.begin();
        lunarisGlobal.update('@store1', insertedObjs[0]);
        lunarisGlobal.insert('@multiple1', [
          { id : 1, label : 'A' },
          { id : 2, label : 'B' }
        ]);
    ...
    it.skip('should rollback a store : DELETE', done => {
      ...
      lunarisGlobal.hook('inserted@store1', (insertedObjs) => {
        ...
        lunarisGlobal.begin();
        lunarisGlobal.delete('@store1', insertedObjs[0]);
        lunarisGlobal.insert('@multiple1', [
          { id : 1, label : 'A' },
          { id : 2, label : 'B' }
        ]);
    ...
    it.skip('should not rollback a store : GET', done => {
      ...
      lunarisGlobal.hook('update@multiple1', () => {
        _events.push('update@multiple1');
      });
      lunarisGlobal.hook('updated@multiple1', () => {
        _events.push('updated@multiple1');
      });
      lunarisGlobal.hook('delete@multiple1', () => {
        _events.push('delete@multiple1');
      });
      lunarisGlobal.hook('deleted@multiple1', () => {
        _events.push('deleted@multiple1');
      });

      lunarisGlobal.begin();
      lunarisGlobal.get('@multiple1');
      lunarisGlobal.commit(() => {
        should(_events).eql([]);
        done();
      });
