...
function _compileStores (varStores, options) {
  ...
  for (let i = 0; i < _storeNames.length; i++) {
    ...
    try {
      ...
      _store.code  += '\nexports["defaultValue"] = ' + serializer(_compilation.defaultValue) + ';';
      _store.code  += '\nexports["aggregateFn"] = ' + serializer(_compilation.aggregateFn) + ';';
      _store.code  += '\nexports["onValidate"] = ' + serializer(_compilation.onValidate) + ';';
