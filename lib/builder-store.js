...
function _compileStores (varStores, options) {
  ...
  for (let i = 0; i < _storeNames.length; i++) {
    ...
    try {
      ...
      _store.code  += '\nexports["defaultValue"] = ' + serializer(_compilation.meta.defautlValue) + ';';
      _store.code  += '\nexports["aggregateFn"] = ' + serializer(_compilation.meta.aggregateFn) + ';';
      _store.code  += '\nexports["onValidate"] = ' + serializer(_compilation.meta.onValidate) + ';';
