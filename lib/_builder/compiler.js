...
function _compileVueFile (fileContent) {
  ...
  return _compileVuejsTemplate(_res, _template);
}
...
function compileVuejsTemplates (id, fileContent, options = {}) {
  ...
  var _isTranslated = options.isTranslated || false;

  try {
    if (/\.vue$/.test(id)) {
      return _compileVueFile(fileContent);
    }
    ...
    if (/\.html$/.test(_template)) {
      ...
      if (!cachedModules[id] || !isProduction) {
      ...
      _subTemplate = translate(_langPath, _lang, _subTemplate, _isTranslated);
...
function compiler (id, file, options) {
  ...
  for (var i = 0; i < _imports.length; i++) {
    ...
    _file.childs[_path] = compiler(_path, fs.readFileSync(_file.imports[_imports[i]]).toString(), options);
  }
