 ...
 * @return {String}
 */
function _compileVueFile (fileContent, isProduction = false) {
...
function compileVuejsTemplates (id, fileContent, options = {}) {
  ...
  try {
    if (/\.vue$/.test(id)) {
      fileContent = _compileVueFile(fileContent, _isProduction);
    }
    ...
    if (/\.html$/.test(_template)) {
      ...
      _template = translate(_langPath, _lang, _template, _isTranslated);
