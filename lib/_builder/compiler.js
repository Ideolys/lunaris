...
function _setCache (id, data) {
  cachedModules[id] = JSON.parse(JSON.stringify(data));
}

function _getCache (id) {
  if (cachedModules[id]) {
    return JSON.parse(JSON.stringify(cachedModules[id]));
  }
}
...
function _compile (id, file, options, errors, callback) {
  if (cachedModules[id]) {
    return callback(errors, _getCache(id));
  }
  ...
  utils.genericQueue(_imports, (importPath, next) => {
    ...
    if (!cachedModules[id] && options.isProduction) {
      _setCache(id, _file);
    }
