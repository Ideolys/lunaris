 ...
 * @returns {String}
 */
function getCode (code, fors, level, isReturnABoolean) {
  var _level    = level || 0;
 ...
 */
function getReflexiveFn (compilation, reflexiveKey) {
  if (!reflexiveKey) {
    return null;
  }
  ...
  function _getFors (type) {
    var _fors       = {};
    var _keyAndFor  = getKey(_fors, 0, compilation, {}, reflexiveKey);
    var _code       = '';
    ...
    if (!_keyAndFor.for) {
      ...
      if (type === 'update') {
        var _forsParent      = {};
        var _keyParentAndFor = getKey(_forsParent, 0, compilation, {}, reflexiveKey, 'objectParent');
        _parentCode          = 'delete ' + _keyParentAndFor.key + ';\n';
        if (_keyAndFor.for) {
          _keyAndFor.for.code.push(_parentCode);
        }
        _parentCode = getCode(_parentCode, _forsParent, 1);
      }
  ...
  return {
    update : new Function('primaryKeyFn', 'objectParent', 'object', getCode.apply(null, _getFors('update'))),
    delete : new Function('primaryKeyFn', 'objectParent', 'object', getCode.apply(null, _getFors('delete')))
  };
}
