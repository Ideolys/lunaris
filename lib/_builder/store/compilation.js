...
function getKey (fors, nbFor, compilation, virtualCompilation, key, objName, parameters) {
  ...
  var _lastObject         = objName    || 'object';
  ...
  return { key : _key, for : _currentFor, lastArray : _lastArray, lastObject : _lastObject, isObjKeyFound : _keyHasBeenFound, type : _type };
}
...
function getFilterFns (ignoredStores, compilatedStores, compilation, filters) {
  ...
  for (var i = 0; i < filters.length; i++) {
    ...
    if (filters[i].isOffline === false || filters[i].isSearchable === false) {
      continue;
    }
