...
const schema = {
  ...
  analyzeDescriptor : function (obj) {
    ...
    return {
      ...
      getJoinFns    : _getJoinFns(_objDesc, _meta.joins)
    };
  }
    ...
    if (typeof _item === 'string') {
      ...
      // detect database primary keys column names (double chevron. ex : <<idMenu>>)
        ...
        if (_store) {
          ...
          meta.aggregates[_jsonUniqueName] = [_item, _column.replace('@' + _store.store, _storeAttr)];
        }
        else {
 ...
 * @param {Object} fns
 */
function _getJoinFns (compilation, joins) {
    ...
    for (var i = 0; i < _joinedStores.length; i++) {
      ...
      for (var l = 0; l < _attributePathParts.length; l++) {
        ...
        if (_compilationObj && _compilationObj.type === 'array') {
          ...
          else if (!_currentFor.children[_keyToJoinedAttribute]) {
        ...
        if (l === _attributePathParts.length - 1) {
          if (type === 'set') {
            var _codeInter = `${ _keyToJoinedAttribute } = joinedValues['${ _joinedStores[i] }'];\n`;
            if (!_currentFor) {
              _code += `
                if (${ _previousKey }) {
                  ${ _codeInter }
              ...
              _currentFor.code.push(_codeInter);
            }
          }

          if (type === 'insert') {
            _codeInter = `
              if (!Array.isArray(${ _keyToJoinedAttribute })) {
  ...
  var _functions = {
    set : new Function ('object', 'joinedValues', _getCode.apply(null, _getFors(joins, 'set')))
  };
  ...
  for (var j = 0; j < _joinedStores.length; j++) {
    ...
    _functions[_joinedStores[j]] = {
      insert : new Function ('object', 'joinedValue', _getCode.apply(null, _getFors(_join, 'insert'))),
      delete : new Function ('object', 'joinedValue', _getCode.apply(null, _getFors(_join, 'delete')))
    };
  }
