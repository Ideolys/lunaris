...
function _getAggregateFn (compilation, aggregates, sortedAggregates) {
  ...
  return new Function('object', 'aggregates', 'constants', _getCode(_code, _fors));
}
 ...
 */
function _getCodeForAggregateValues (aggregateType, keyToAggregate, keyToAggregateState, keyToAggregateValues, functionType, transformerFn, transformerFnParameters) {
  var _res = {
    code       : '',
    codeBefore : ''
  };
        ...
        if (l === _attributePathParts.length - 1) {
          ...
          if (type === 'insert') {
            ...
            if (externalAggregates[_joinedStores[i]]) {
              ...
              _aggregateState    = _getAggregateStateKey(externalAggregates[_joinedStores[i]][1], _keyForToAggregate.key, null, _parameters);
...
function _getComputedPropertiesFn (compilation, computedFns) {
  ...
  return new Function('object', 'constants', _getCode(_code, _fors));
}
