...
const schema = {
  ...
  analyzeDescriptor : function (obj) {
    ...
    var _onValidate  = {};
    var _onTransform = {};
    var _meta        = { sortGroup : {}, jsonToSQL : {}, sortMandatory : [] };
    ...
    _objDesc.main0            = {
      ...
      name         : '',
      type         : _mainType,
      defaultValue : _mainType === 'array' ? [] : null,
      obj          : {},
      objTrans     : {},
      level        : _arrLevel,
      keys         : []
    ...
    for (var _attrName in _analyzedObj) {
      ...
      analyzeRecursive(_attrName, _value, false);
      _arrLevel = 0;
    }

    // Recursive function
    function analyzeRecursive (attrName, attrValue, comingFromArray) {
      ...
      // analyze the nested object (we may come from an array or an object)
        ...
        _objDesc[_nextObjName] = {
          ...
          defaultValue : comingFromArray ? []      : null,
          obj          : {},
          objTrans     : {},
          level        : _arrLevel,
          keys         : []
        ...
        for (var _attrName in attrValue) {
          analyzeRecursive(_attrName, attrValue[_attrName], false);
      ...
      // if the attribute value is a simple array
        ...
        parseArray(attrName, attrValue, _objDesc[_currentObjName], _currentObjName, _onValidate, _onTransform, _objDesc, _meta, _nbArr);
      }
  ...
  for (var i = 0; i<attrArray.length; i++) {
    ...
    if (typeof _item === 'string') {
      ...
      // detect database primary keys column names (double chevron. ex : <<idMenu>>)
        ...
        _jsonUniqueName                 = generateJsonUniqueName(objDesc, currentObjName, attrName);
    ...
    if (currentObj.defaultValue === undefined) {
      currentObj.defaultValue = null;
    }
  }
...
function generateJsonUniqueName (objDesc, currentObjName, attrName) {
  ...
  while (_parent !== '' && objDesc[_parent].name !== '') {
    if (objDesc[_parent].type==='object') {
  ...
  // replace .menu by [menu], syntax of IJS
  _jsonUniqueName = _jsonUniqueName.replace(/\.((?:(?!\.|\[\])[\s\S])+)/g, (m, code) => {
    return '[' + code + ']';
  });
