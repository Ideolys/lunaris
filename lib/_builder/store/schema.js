...
const schema = {
  ...
  getAggregateFn : _getAggregateFn,
  ...
  analyzeDescriptor : function (obj) {
    ...
    function analyzeRecursive (attrName, attrValue, comingFromArray, defaultValue) {
      ...
      // If it is an object
        ...
        analyzeRecursive(attrName, _lastElement, false, {});
        defaultValue[attrName] = null;
      }
      ...
      // if the attribute value is a simple array
        ...
        defaultValue[attrName] = parseArray(attrName, attrValue, _objDesc[_currentObjName], _currentObjName, _onValidate, _onTransform, _objDesc, _meta, _nbArr, _aggregateAttributes);
      }
 ...
 */
function parseArray (attrName, attrArray, currentObj, currentObjName, onValidate, onTransform, objDesc, meta, currentArrayNumber, aggregateAttributes) {
      ...
      // detect database primary keys column names (double chevron. ex : <<idMenu>>)
        ...
        if (_store) {
          ...
          objDesc[_storeAttr] = {
            ...
            objParent  : currentObj.type === 'object' ? currentObjName : '',
            name       : _storeAttr,
            type       : 'array',
            obj        : {},
            objTrans   : {},
          ...
          if (_indexFound !== -1) {
            ...
            meta.externalAggregates[_stores[_indexFound]] = [_item, _jsonUniqueName, _column];

            objDesc[_stores[_indexFound]] = {
              arrChild   : [],
              arrParents : currentObj.type === 'array'  ? currentObjName : '',
              objParent  : currentObj.type === 'object' ? currentObjName : '',
              name       : _stores[_indexFound],
              type       : 'array',
              obj        : {},
              objTrans   : {},
              level      : currentObj.level,
        ...
        if (_indexFound !== -1) {
          ...
          meta.externalAggregates[_stores[_indexFound]] = [meta.aggregates[aggregateAttributes[_jsonUniqueName]][0], aggregateAttributes[_jsonUniqueName], meta.aggregates[aggregateAttributes[_jsonUniqueName]][1]];
          delete meta.aggregates[_jsonUniqueName];
 ...
 * @returns {String}
 */
function _getCode (code, fors, level) {
  var _level    = level || 0;
 ...
 * @param {Object} joins [{ store : key }]
 * @return {Function}
 */
function _getAggregateFn (compilatedStores, compilation, aggregates, sortedAggregates, joins) {
  var _fors  = {};
  var _joins = Object.values(joins);

  for (var i = 0; i < sortedAggregates.length; i++) {
    if (aggregates[sortedAggregates[i]][1][0] !== '@') {
      ...
      for (var j = 0; j < _aggregateAttributesPathParts.length; j++) {
        ...
        if (j === _aggregateAttributesPathParts.length - 1) {
          console.log(_aggregateAttributesPathParts, _keyToAggregateValue);
  ...
  return new Function('object', 'aggregates', _getCode(null, _fors));
}
 ...
 * @returns {String}
 */
function _getKey (fors, nbFor, compilation, key) {
  ...
  return { key : _key, for : _currentFor };
}
 ...
 */
function _getCodeForAggregateValues (aggregateType, keyToAggregate, keyToAggregateValues, functionType) {
  var _res = {
    code       : '',
    codeBefore : ''
  };
  ...
  if (functionType === 'set') {
    _res.code = `
      var _aggregateValue = ${ keyToAggregate } || _aggregate.getStartValue();
    `;
    _res.codeBefore = `
      var _aggregate      = aggregates['${ aggregateType }'];
      var _aggregateValue = ${ keyToAggregate } || _aggregate.getStartValue();
      ${ keyToAggregate } = _aggregateValue;
    `;

    _res.code += `
      ${ keyToAggregate } = _aggregate.add(_aggregateValue, ${ keyToAggregateValues });
    `;
  ...
  if (functionType === 'insert') {
    _res.code = `
      var _aggregateValue = ${ keyToAggregate } || undefined;
      var _aggregate      = aggregates['${ aggregateType }'];
      ${ keyToAggregate } = ${ keyToAggregate } || _aggregate.getStartValue();
      ${ keyToAggregate } = _aggregate.add(_aggregateValue, ${ keyToAggregateValues });
    `;
  ...
  _res.code = `
      ...
      var _aggregate      = aggregates['${ aggregateType }'];
      ${ keyToAggregate } = ${ keyToAggregate } || _aggregate.getStartValue();
      ${ keyToAggregate } = _aggregate.remove(_aggregateValue, ${ keyToAggregateValues });
    `;
 ...
 * @returns {Object} fns
 */
function _getJoinFns (compilatedStores, compilation, joins, externalAggregates) {
        ...
        if (l === _attributePathParts.length - 1) {
          if (type === 'set') {
            ...
            if (externalAggregates[_joinedStores[i]]) {
              _keyForToAggregate         = _getKey(_fors, _nbFor, compilation, externalAggregates[_joinedStores[i]][1]);
              var _keyForAggregateValues = _getKey(_fors, _nbFor, compilation, externalAggregates[_joinedStores[i]][2]);
              var _aggregateCode         = _getCodeForAggregateValues(externalAggregates[_joinedStores[i]][0], _keyForToAggregate.key, _keyForAggregateValues.key, type);
          ...
          if (type === 'insert') {
            ...
            if (externalAggregates[_joinedStores[i]]) {
              ...
              _aggregateCode     = _getCodeForAggregateValues(externalAggregates[_joinedStores[i]][0], _keyForToAggregate.key, _value, type);
            }
            if (compilatedStores[_joinedStores[i]] && compilatedStores[_joinedStores[i]].isStoreObject) {
              _codeInter = `
  ...
  var _functions = {
    set : new Function ('object', 'joinedValues', 'aggregates', _getCode.apply(null, _getFors(joins, 'set')))
  };
  ...
  for (var j = 0; j < _joinedStores.length; j++) {
    ...
    _functions[_joinedStores[j]] = {
      insert : new Function ('object', 'joinedValue', 'aggregates', _getCode.apply(null, _getFors(_join, 'insert'))),
      delete : new Function ('object', 'joinedValue', 'aggregates', _getCode.apply(null, _getFors(_join, 'delete')))
    };
  }
