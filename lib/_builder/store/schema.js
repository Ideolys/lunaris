...
const schema = {
  ...
  analyzeDescriptor : function (obj, storeName) {
    ...
    return {
      ...
      setPrimaryKey      : _setPrimaryKeyFn(_meta.primaryKey),
 ...
 * @return {Function}
 */
function _setPrimaryKeyFn (primaryKey) {
  var _fn = 'var _pk = \'\';';
  ...
  for (var i = 0; i < primaryKey.length; i++) {
    _fn += `
      _pk += (item['${ primaryKey[i] }'] || val) + '-';
      if (!item['${ primaryKey[i] }']) {
        item['${ primaryKey[i] }'] = '_' + val;
      }
    `;
  }
 ...
 * @returns {Object}
 */
function _findCompilationObject (compilation, attribute, parent) {
...
function _getKey (fors, nbFor, compilation, virtualCompilation, key, objName, parameters) {
  ...
  for (var i = 0; i < _pathParts.length; i++) {
    _key += `['${ _pathParts[i] }']`;
    var _compilationObj = _findCompilationObject(compilation, _pathParts[i], _lastCompilationObj ? _lastCompilationObj.name : '');
    if (!_compilationObj) {
  ...
  return { key : _key, for : _currentFor, lastArray : _lastArray, isObjKeyFound : _keyHasBeenFound };
}
 ...
 * @return {Object}
 */
function _getFilterFns (ignoredStores, compilatedStores, compilation, filters) {
  var _filtersFns = {};
  ...
  for (var i = 0; i < filters.length; i++) {
    ...
    if (filters[i].localAttribute && /\[|\]/.test(filters[i].localAttribute)) {
      throw new Error('A filter local attribute must not contain "[" or "]", use "." notation instead : path.to.my.element');
    }
    ...
    if (!_keyAndCode.isObjKeyFound) {
      throw new Error('A filter must have a local attribute defined in the map! Error in store.filters[' + i + ']');
    }
    ...
    if (_operator !== OPERATORS['ILIKE']) {
      _filtersFns[_filterKey] = new Function('filterValue', 'object', 'fn', _getCode(_code, _fors, !_keyAndCode.for, true));
    }
    else {
      _filtersFns[_filterKey] = new Function('filterValue', 'object', 'fn', _getCode(_code, _fors, !_keyAndCode.for, true));
    }
