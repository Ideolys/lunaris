...
function _getJoinFns (compilatedStores, compilation, virtualCompilation, joins, externalAggregates) {
    ...
    for (var i = 0; i < _joinedStores.length; i++) {
      ...
      for (var l = 0; l < _attributePathParts.length; l++) {
        ...
        if (l === _attributePathParts.length - 1) {
          ...
          if (type === 'delete') {
            _parameters        = [_keyToJoinedAttribute + '[i]'];
            _keyForToAggregate = null;
            _aggregateCode     = null;
            if (externalAggregates[_joinedStores[i]]) {
              _keyForToAggregate = _getKey(_fors, _nbFor, compilation, virtualCompilation, externalAggregates[_joinedStores[i]][1]);
  ...
  var _functions = {
    set : new Function ('object', 'joinedValues', 'aggregates', 'constants', _getCode.apply(null, _getFors(joins, 'set')))
  };
  ...
  for (var j = 0; j < _joinedStores.length; j++) {
    ...
    _functions[_joinedStores[j]] = {
      insert : new Function ('object', 'joinedValue', 'aggregates', 'constants', _getCode.apply(null, _getFors(_join, 'insert'))),
      delete : new Function ('object', 'joinedValue', 'aggregates', 'constants', _getCode.apply(null, _getFors(_join, 'delete')))
    };
  }
