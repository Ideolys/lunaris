...
const schema = {
  ...
  analyzeDescriptor : function (obj) {
    ...
    return {
      ...
      aggregateFn        : _getAggregateFn(_objDesc, _meta.aggregates, _meta.aggregatesSort)
    };
  }
 ...
 * @return {Function}
 */
function _getAggregateFn (compilation, aggregates, sortedAggregates) {
  var _fors = {};
  var _code = '';
  ...
  for (var i = 0; i < sortedAggregates.length; i++) {
    ...
    for (var l = 0; l < _attributePathParts.length; l++) {
      ...
      if (l === _attributePathParts.length - 1) {
        var _keyAndForToAggregateValue = _getKey(_fors, _nbForToAggregateValue, compilation, {}, aggregates[_currentAttribute][1]);
        _keyAndForToAggregateValue.for.code.push(`
          ...
          ${ _keyToAggregateValue } = _aggregate.add(_aggregateValue, ${ _keyAndForToAggregateValue.key });
        `);
        ...
        // Reinit values
        var _resetStr = `${ _keyToAggregateValue } = undefined;`;
        if (!_currentFor) {
          _code += _resetStr;
        }
        else {
  ...
  return new Function('object', 'aggregates', _getCode(_code, _fors));
}
 ...
 * @returns {Object} { key : {String}, for : {Object}, lastArray : {String} }
 */
function _getKey (fors, nbFor, compilation, virtualCompilation, key) {
  var _pathParts  = key.split('.');
  var _key        = 'object';
  var _lastArray  = 'object';
  var _currentFor = null;
  var _nbFor      = 0;
 ...
 * @returns {Object} fns
 */
function _getJoinFns (compilatedStores, compilation, virtualCompilation, joins, externalAggregates) {
        ...
        if (l === _attributePathParts.length - 1) {
          if (type === 'set') {
            ...
            if (externalAggregates[_joinedStores[i]]) {
              ...
              var _keyForAggregateValues = _getKey(_fors, _nbFor, compilation, virtualCompilation, externalAggregates[_joinedStores[i]][2]);
