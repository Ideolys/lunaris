...
function _getAggregateFns (compilation, aggregates, sortedAggregates) {
  ...
  for (var i = 0; i < sortedAggregates.length; i++) {
    ...
    for (var l = 0; l < _attributePathParts.length; l++) {
      ...
      if (compilation[_attributePathParts[l] + (l + 1)] && compilation[_attributePathParts[l] + (l + 1)].type === 'array') {
        _keyToAggregateValue += '[_' + (l + 1) + ']';
      }
    }
    ...
    for (var j = 0; j < _aggregateAttributesPathParts.length; j++) {
      ...
      if (compilation[_currentAggregateAttributePathPart + (j + 1)] && compilation[_currentAggregateAttributePathPart + (j + 1)].type === 'array') {
        _nbFor++;

        if (!_fors[_keyToValue]) {
          _fors[_keyToValue] = { code : [], codeAfterFor : [] };
        }
        _keyToArrayToAggregate = _keyToValue;
        _keyToValue           += '[_' + _nbFor + ']';
      }

      if (j === _aggregateAttributesPathParts.length - 1) {
        _fors[_keyToArrayToAggregate].code.push(`
        ...
        `);
        _fors[_keyToArrayToAggregate].codeAfterFor.push(`
  ...
  var _forKeys  = Object.keys(_fors);
  var _forsCode = '';
  for (var k = _forKeys.length - 1; k >= 0; k--) {

    _forsCode = `if (${ _forKeys[k] }) {\n for (var _${ k + 1 } = 0; _${ k + 1 } < ${ _forKeys[k] }.length; _${ k + 1}++) {\n` + _forsCode;
    _forsCode += _fors[_forKeys[k]].code.join('\n') + '}\n';
    _forsCode += '}\n'; // close the if
    _forsCode += _fors[_forKeys[k]].codeAfterFor.join('\n');
  }
  ...
  return new Function('object', 'previousObject', 'aggregates', _fn);
}
