...
const schema = {
  ...
  analyzeDescriptor : function (obj, storeName) {
    ...
    return {
      ...
      computedsFn        : _getComputedPropertiesFn(_objDesc, _meta.computedFns)
    };
  }
    ...
    for (var l = 0; l < _attributePathParts.length; l++) {
      ...
      if (l === _attributePathParts.length - 1) {
        ...
        _keyAndForToAggregateValue.for.code.push(`
          ...
          ${ _aggregateState }      = _aggregate.add(${ _aggregateState }, ${ _keyAndForToAggregateValue.key });
 ...
 */
function _getKey (fors, nbFor, compilation, virtualCompilation, key, objName) {
 ...
 */
function _getCodeForAggregateValues (aggregateType, keyToAggregate, keyToAggregateState, keyToAggregateValues, functionType) {
  var _res = {
    code       : '',
    codeBefore : ''
  };
  ...
  if (functionType === 'set') {
    ...
    _res.code += `
      ${ keyToAggregateState } = _aggregate.add(_aggregateState, ${ keyToAggregateValues });
  ...
  _res.code = `
      var _aggregateState      = ${ keyToAggregateState } || null;
      var _aggregate           = aggregates['${ aggregateType }'];
      ${ keyToAggregateState } = _aggregate.remove(_aggregateState, ${ keyToAggregateValues });
      ${ keyToAggregate }      = ${ keyToAggregateState }.value;
    `;
        ...
        if (l === _attributePathParts.length - 1) {
          if (type === 'set') {
            ...
            if (externalAggregates[_joinedStores[i]]) {
              ...
              var _aggregateCode         = _getCodeForAggregateValues(externalAggregates[_joinedStores[i]][0], _keyForToAggregate.key, _aggregateState, _keyForAggregateValues.key, type);
          ...
          if (type === 'insert') {
            ...
            if (externalAggregates[_joinedStores[i]]) {
              ...
              _aggregateState    = _getAggregateStateKey(externalAggregates[_joinedStores[i]][1], _keyForToAggregate.key);
              _aggregateCode     = _getCodeForAggregateValues(externalAggregates[_joinedStores[i]][0], _keyForToAggregate.key, _aggregateState, _value, type);
            }
            if (compilatedStores[_joinedStores[i]] && compilatedStores[_joinedStores[i]].isStoreObject) {
              _codeInter = `
...
/**
 * Get computed properties function
 * @param {Object} compilation
 * @param {Object} computedFns { attrbiutePath : function }
 * @returns {Function}
 */
function _getComputedPropertiesFn (compilation, computedFns) {
