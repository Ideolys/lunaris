...
function getRoutes(options){
  var routes      = {};
  var _groupsPath = path.join(options.clientPath, 'groups.json');

  if(!fs.existsSync(_groupsPath)){
    return console.log('Specify your groups in client/groups.json');
  }
  ...
  routes         = translate(options.langPath, options.lang, _routesTxt);
