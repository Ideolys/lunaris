  ...
  walkStores(buildOptions.storesFolder, (err, stores) => {
    ...
    for (var i = 0; i < stores.length; i++) {
      try {
        ...
        _code      += `lunaris._stores['${ _storeName[2] }'] = ${ _sanitizePath(stores[i]) };\n`;
...
function _buildCode (modules, sort, isLunarisStore) {
  ...
  for (var i = 0; i < sort.length; i++) {
    ...
    var _lunarisInit = `
      ...
      exports['data_filtered']         = lunaris._collection();
      ...
      exports._registerFilterHooks = function () {
        for (i = 0; i < exports.filters.length; i++) {
          ...
          if (exports.filters[i].isAutoUpdate === undefined || exports.filters[i].isAutoUpdate === true) {
            lunaris.hook('update@' + exports.filters[i].source, _handler);
            lunaris.hook('insert@' + exports.filters[i].source, _handler);
            lunaris.hook('reset@'  + exports.filters[i].source, _handler);
          }
        }
      };
    `;
  ...
  buildGlobalComponents(buildOptions, globalComponentsCode => {
    buildModules(buildOptions, (modulesCode, paths) => {
      buildStores(buildOptions, storesCode => {
        ...
        var _indexApp = fs.readFileSync(path.join(buildOptions.modulesFolder, '..', 'index.js')) || _defautlIndexApp;
        _code = vuejsIndex
