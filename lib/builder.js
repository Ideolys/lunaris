  ...
  walkModules(buildOptions.modulesFolder, (err, modules) => {
    ...
    utils.genericQueue(modules, (moduleToCompile, next) => {
      ...
      utils.genericQueue(_paths, (pathModule, nextSecond) => {
        ...
        let filename = moduleToCompile[1][pathModule].controllers;
        if (!/\.(vue|js)$/m.test(filename)) {
          filename += '.js';
        }

        var _path = path.join(moduleToCompile[0], filename);
...
function build (buildOptions, callback) {
  ...
  buildGlobalComponents(buildOptions, globalComponentsCode => {
    buildModules(buildOptions, (modulesCode, paths, moduleRefs) => {
      buildStores(buildOptions, storeNames, compilatedStores => {
        _code += globalComponentsCode;
        _code += modulesCode;
        _code += compilatedStores.code;
          ...
          getLunarisBuild(buildOptions, (err, lunarisBuild) => {
            ...
            _code = vuejsIndex
              ...
              .replace(/\{\{__LUNARIS_OPTIONS__\}\}/g, JSON.stringify({
                ...
                urlsGraph           : compilatedStores.urlsGraph,
                constants           : buildOptions.constants,
                cacheGraph          : compilatedStores.cacheGraph,
...
function _setLunarisDeps (buildOptions) {
  ...
  _path          = path.join(_vuejsPath, buildOptions.isProduction ? 'vue.min.js' : 'vue.js');
  ...
  if (fs.existsSync(path.join(dayjs, 'locale', buildOptions.lang.toLowerCase() + '.js'))) {
    _path = path.join(dayjs, 'locale', buildOptions.lang.toLowerCase() + '.js');
  }
  ...
  buildOptions.dependencies.push(path.join(dayjs, 'plugin', 'localizedFormat.js'));
  buildOptions.dependencies.push(path.join(dayjs, 'plugin', 'localeData.js'));
  buildOptions.dependencies.push(path.join(dayjs, 'plugin', 'customParseFormat.js'));
  buildOptions.dependencies.push(path.join(dayjs, 'plugin', 'isLeapYear.js'));
  buildOptions.dependencies.push(path.join(dayjs, 'plugin', 'utc.js'));
  buildOptions.dependencies.push(path.join(dayjs, 'plugin', 'minMax.js'));
}
