...
function buildStores (buildOptions, callback) {
  ...
  walkStores(buildOptions.storesFolder, (err, stores) => {
    ...
    for (var i = 0; i < stores.length; i++) {
      try {
        ...
        if (_storeCodeEval.map) {
          ...
          _storesPath[_path]               = _storeName[2];
    ...
    for (i = 0; i < _storesPathKey.length; i++) {
      var _compilation = _compilatedStores[_storesPath[_storesPathKey[i]]];
      ...
      for (var k = 0; k < _storesToJoin.length; k++) {
        ...
        _graphJoins[_storesToJoin[k]].push(_storeCodeEval.name);

        _joinDescriptor['collections'][_storesToJoin[k]] = 'lunaris._stores[\'' + _storesToJoin[k] + '\'].data';
      }
      _joinDescriptor['joinFns'] = storeMap.getJoinFns(_compilatedStores, _compilation.compilation, _compilation.meta.joins);
      _stores[_storesPathKey[i]].code += '\nexports["joins"] = ' + JSON.stringify(_joinDescriptor) + ';';
    }
    ...
    _code = _buildCode(_graphInfos.flattenedObjects, _graphInfos.order, true, _graphJoins) + _code + _codeAfter;
    callback(_code);
  });
}
