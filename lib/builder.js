  ...
  walkStores(buildOptions.storesFolder, (err, stores) => {
    ...
    for (var i = 0; i < stores.length; i++) {
      try {
        ...
        if (_storeCodeEval.length === 2) console.log(_storeCodeEval.length);
        for (var k = 0; k < _storeCodeEval.length; k++) {
          ...
          if (_store.filters) {
            for (var j = 0; j < _store.filters.length; j++) {
              if (!_graph[_store.filters[j].source]) {
                _graph[_store.filters[j].source] = [];
              }
              _graph[_store.filters[j].source].push(_store.name);
            }
          }
          ...
          _codeAfter += `lunaris._stores['${ _store.name }']._registerFilterHooks();\n`;
        }
      }
      catch (e) {
    ...
    for (i = 0; i < _storeNames.length; i++) {
      ...
      try {
        ...
        _errors.push('[Builder] Error when compiling the store "' + _storeNames[i] + '", look at your terminal!');
        console.log('\n', '[Builder] Error when compiling the store "' + _storeNames[i] + '"', e);
      }
    }
    ...
    _code = _vars + _buildCode(_graphInfos.flattenedObjects, _graphInfos.order, true, _graphJoins, true)[0] + _code + _codeAfter;
 ...
 */
function _buildCode (modules, sort, isLunarisStore, storeJoins, isUnderscoreBeforeVar = false) {
  var _code    = '';
  var _modules = '{';
