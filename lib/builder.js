      ...
      for (var j = 0; j < _paths.length; j++) {
        if (_isModuleAuthorized(buildOptions.profile, modules[i][0]) || buildOptions.isLangGeneration) {
          try {
            ...
            _pathUrls.push([
              _paths[j],
              _path,modules[i][1][_paths[j]].name,
              (modules[i][1][_paths[j]].description || ''),
              (modules[i][1][_paths[j]].isOffline   || false)
            ]);
          }
          catch (e) {
            console.log(e);
  ...
  walkStores(buildOptions.storesFolder, (err, stores) => {
    ...
    for (var i = 0; i < stores.length; i++) {
      try {
        ...
        for (var k = 0; k < _storeCodeEval.length; k++) {
          ...
          _stores[_store.name].isMapClone = _store.isMapClone;
    ...
    for (i = 0; i < _storeNames.length; i++) {
      ...
      try {
        ...
        _store.code += '\nexports["collections"] = ' + serializer(_collections) + ';';
