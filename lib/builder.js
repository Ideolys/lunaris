    ...
    walkModules(buildOptions.modulesFolder, (err, modules) => {
      ...
      const _rolluptInputOptions = {
        ...
        plugins : [
          ...
          compiler({
            ...
            langPath     : buildOptions.langPath,
            lang         : buildOptions.lang,
            isTranslated : buildOptions.isLangGeneration
          })
        ]
      };
      ...
      rollup.rollup(_rolluptInputOptions).then(bundle => {
        const result = bundle.generate({
          ...
          result.code = translate(buildOptions.langPath, buildOptions.lang, result.code, buildOptions.isLangGeneration);
          result.code = transpile(result.code);
...
function _insertDependencies (js, options) {
  ...
  // vuejs
  const _vuejsPath = require.resolve('vue');
  const _vuejsDir  = _vuejsPath.split('/').slice(0, -1).join('/');
  const _vueBuild  = options.isProduction ? 'vue.min.js' : 'vue.js';
  _path           = path.join(_vuejsDir, _vueBuild);
