...
function _compileStores (varStores, errors, options) {
  var _graphJoins       = {};
...
function buildStores (buildOptions, storeNames, callback) {
  ...
  walkStores(_options.storesFolder, (err, stores) => {
    ...
    () => {
      let errors = [];
      let _code  = _compileStores(varStores, errors, _options);
 ...
 * @returns {String}
 */
function buildLunaris (externals, callback) {
      ...
      buildStores(buildOptions, storeNames, (storesCode, compilationErrors) => {
        ...
        compiler(_indexAppOfflinePath, _indexAppOffline, buildOptions, (err, res) => {
          ...
          buildLunaris({
            ...
            _code = vuejsIndex
              .replace(/\{\{__LUNARIS__\}\}/g, lunarisCode)
