    ...
    var _lunarisInit = `
      ...
      exports._registerFilterHooks = function () {
        for (i = 0; i < exports.filters.length; i++) {
          ...
          lunaris.hook('update' + _filter, _handler);
          lunaris.hook('insert' + _filter, _handler);
 ...
 * @returns {String}
 */
function buildLunaris (buildOptions, externals) {
  ...
  buildGlobalComponents(buildOptions, globalComponentsCode => {
    buildModules(buildOptions, (modulesCode, paths) => {
      buildStores(buildOptions, storesCode => {
        ...
        _code = vuejsIndex
                      .replace(/\{\{__LUNARIS__\}\}/g, buildLunaris(buildOptions, { isProduction : buildOptions.isProduction }))
