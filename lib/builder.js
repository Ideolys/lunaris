  ...
  walkStores(buildOptions.storesFolder, (err, stores) => {
    ...
    for (var i = 0; i < stores.length; i++) {
      try {
        ...
        if (Object.values(storeNames).indexOf(_storeName[2]) !== -1) {
          throw new Error('The store "' + _storeName[2] + '" already exists!');
        }
        ...
        if (_storeCodeEval.map) {
          ...
          var _isStoreObject = !Array.isArray(_storeCodeEval.map);
          _storeCode += '\nexports["isStoreObject"] = ' + _isStoreObject + ';';
          var _meta                        = storeMap.analyzeDescriptor(_storeCodeEval.map, _storeName[2]);
          _meta.isStoreObject              = _isStoreObject;
          _storesPath[_storeName[2]]       = _path;
          _compilatedStores[_storeName[2]] = _meta;
          _storeCode += '\nexports["reflexive"] = ' + _stringifyObject(_meta.reflexiveFn) + ';';
          ...
          _compilatedStores[_storeName[2]].filters = _storeCodeEval.filters;
          _compilatedStores[_storeName[2]].path    = stores[i];
        }
        else {
          _storeCode += '\nexports["isStoreObject"] = false;';
          _storeCode += '\nexports["filtersFns"]    = null;';
        ...
        _codeAfter += `lunaris._stores['${ _storeName[2] }']._register();\n`;
      }
      catch (e) {
        _errors.push('[Builder] Error when compiling the store "' + stores[i] + '", look at your terminal!');
        console.log('\n', '[Builder] Error when compiling the store "' + stores[i] + '"', e);
      }
    }

    var _storesPathKey = Object.keys(_storesPath);
    var _storesChanged = [];
    for (i = 0; i < _storesPathKey.length; i++) {
      var _compilation = _compilatedStores[_storesPathKey[i]];
      try {
        var _collections    = '';
        ...
        for (var k = 0; k < _storesToJoin.length; k++) {
          if (!_storesPath[_storesToJoin[k]]) {
          ...
          _graphJoins[_storesToJoin[k]].push(_storesPathKey[i]);
          _collections += _storesToJoin[k] + ' : ' + _storesPath[_storesToJoin[k]] + '.data,' ;

          // Reorder _stores object in order to put joined stores, children of the current stores
          _stores[_storesPath[_storesPathKey[i]]].children[_storesPath[_storesToJoin[k]]] = _stores[_storesPath[_storesToJoin[k]]];
          _storesChanged.push(_storesPath[_storesToJoin[k]]);
        }
        _stores[_storesPath[_storesPathKey[i]]].code += '\nexports["collections"] = {' + _collections + '};';
        _joinDescriptor['joinFns'] = storeMap.getJoinFns(
          ...
          storeNames[_storesPath[_storesPathKey[i]]]
        );
        _stores[_storesPath[_storesPathKey[i]]].code += '\nexports["joins"] = ' + _stringifyObject(_joinDescriptor) + ';';
        _stores[_storesPath[_storesPathKey[i]]].code += '\nexports["joins"]["collections"] = exports["collections"];';
        _stores[_storesPath[_storesPathKey[i]]].code += '\nexports["filterFns"] = ' + _stringifyObject(storeMap.getFilterFns(_compilatedStores, _compilation.compilation, _compilation.filters));
      }
      catch (e) {
        _errors.push('[Builder] Error when compiling the store "' + _compilation.path + '", look at your terminal!');
        console.log('\n', '[Builder] Error when compiling the store "' + _compilation.path + '"', e);
      }
    }
    ...
    _code = _vars + _buildCode(_graphInfos.flattenedObjects, _graphInfos.order, true, _graphJoins, storeNames)[0] + _code + _codeAfter;
 ...
 * @param {Object} storeNames
 */
function _buildCode (modules, sort, isLunarisStore, storeJoins, storeNames = {}) {
  var _code    = '';
  var _modules = '{';
  ...
  for (var i = 0; i < sort.length; i++) {
    ...
    if (isLunarisStore && storeJoins[storeNames[sort[i]]]) {
      storeToPropagate = JSON.stringify(storeJoins[storeNames[sort[i]]]);
    }
...
function _insertDependencies (js, options) {
  ...
  // vue-hammer
  let _vueTouchEvents = require.resolve('vue2-touch-events');
  _vueTouchEvents     = _vueTouchEvents.split('/').slice(0, -1).join('/');
  _path               = path.join(_vueTouchEvents, 'index.js');
  _addDependency(dependenciesCode, _path);
