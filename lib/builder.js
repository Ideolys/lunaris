...
function buildStores (buildOptions, storeNames, callback) {
  ...
  walkStores(buildOptions.storesFolder, (err, stores) => {
    ...
    for (var i = 0; i < stores.length; i++) {
      try {
        ...
        for (var k = 0; k < _storeCodeEval.length; k++) {
          ...
          _stores[_store.name].inherits = _store.inherits ? _store.inherits.replace('@', '') : null;
          _stores[_store.name].filters  = _store.filters;
          _stores[_store.name].path     = stores[i];

          if (_store.from && !/^@/.test(_store.from)) {
            throw new Error('The value in "store.from" must reference a store. Ex : "store.from = \'@myStore\'"');
          }
    ...
    for (i = 0; i < _storeNames.length; i++) {
      ...
      try {
        if (_store.inherits) {
          if (!_stores[_store.inherits]) {
            _store.inherits = null;
            let error = 'The store "' + _store.inherits + '" referenced in "store.inherits" does no exist!';
            throw new Error(error);
          }

          _compilation = _compilatedStores[_store.inherits];
          _store.children[_store.inherits] = _stores[_store.inherits];
        }
        ...
        sendStoreCompilationError(buildOptions.clientFolder, _store.path, _errors, e.message);
      }
    }
  ...
  for (var i = 0; i < sort.length; i++) {
    ...
    var _lunarisInit = `
      ...
      exports._register = function () {
        ...
        if (exports['from']) {
          lunaris.hook('reset'    + exports['from'], _handler, false, true);
          lunaris.hook('get'      + exports['from'], _handler, false, true);
          lunaris.hook('insert'   + exports['from'], _handler, false, true);
          lunaris.hook('inserted' + exports['from'], _handler, false, true);
          lunaris.hook('update'   + exports['from'], _handler, false, true);
          lunaris.hook('updated'  + exports['from'], _handler, false, true);
          lunaris.hook('delete'   + exports['from'], _handler, false, true);
          lunaris.hook('deleted'  + exports['from'], _handler, false, true);
        }
      };
    `;
