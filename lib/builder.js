...
function _buildCode (modules, sort) {
  ...
  for (var i = 0; i < sort.length; i++) {
    var _moduleCode = modules[sort[i]].code;
    ...
    for (var j = 0; j < _importsKeys.length; j++) {
      var _val = _imports[_importsKeys[j]]
                  .replace(/\//g, '_')
                  .replace(/\\/g, '_')
                  .replace(/-/g, '_')
                  .replace(/\./g, '_');

      _moduleCode = _moduleCode.replace(_importsKeys[j], _val + '____export_0');
    }
...
function build (buildOptions, callback) {
  ...
  buildGlobalComponents(buildOptions, (globalComponents) => {
    buildModules(buildOptions, (modules) => {
