...
/**
 * Send store compilation error
 * @param {String} absoluteClientPath
 * @param {String} storePath
 * @param {Array} errors
 * @param {String} error
 */
function sendStoreCompilationError (absoluteClientPath, storePath, errors, error) {
  storePath = storePath.replace(new RegExp(absoluteClientPath, 'g'), '');

  errors.push('[Builder] Error when compiling the store "' + storePath + '" ' + error);
  console.log('\n', '[Error] [Builder] Error when compiling the store "' + storePath + '"', error);
}
...
function buildStores (buildOptions, storeNames, callback) {
  ...
  walkStores(buildOptions.storesFolder, (err, stores) => {
    ...
    for (var i = 0; i < stores.length; i++) {
      try {
        ...
        for (var k = 0; k < _storeCodeEval.length; k++) {
          ...
          if (Object.values(storeNames).indexOf(_store.name) !== -1) {
          ...
          _stores[_store.name].from    = _store.from ? _store.from.replace('@', '') : null;
          _stores[_store.name].filters = _store.filters;
          _stores[_store.name].path    = stores[i];
        ...
        sendStoreCompilationError(buildOptions.clientFolder, stores[i], _errors, e.message);
      }
    }
    ...
    for (i = 0; i < _storeNames.length; i++) {
      ...
      try {
        if (_stores[_storeNames[i]].from) {
          if (!_stores[_stores[_storeNames[i]].from]) {
            _stores[_storeNames[i]].from = null;
            let error = 'The store "' + _stores[_storeNames[i]].from + '" referenced in "from" does no exist!';
            throw new Error(error);
          }

          _compilation = _compilatedStores[_stores[_storeNames[i]].from];
          _stores[_storeNames[i]].children[_stores[_storeNames[i]].from] = _stores[_stores[_storeNames[i]].from];
        }

        _stores[_storeNames[i]].code += '\nexports["computedsFn"] = ' + (_compilation.computedsFn ? _compilation.computedsFn.toString() : null) + ';';
        _stores[_storeNames[i]].code += '\nexports["meta"] = ' + JSON.stringify(_compilation) + ';';
        ...
        for (k = 0; k < _storesToJoin.length; k++) {
          ...
          _collections += _storesToJoin[k] + ' : _' + (_sanitizePath(_storesToJoin[k])) + '_collection,' ;
        ...
        for (let referencedStore in _compilation.meta.references) {
          ...
          _collections += referencedStore + ' : _' + (_sanitizePath(referencedStore)) + '_collection,' ;
        ...
        sendStoreCompilationError(buildOptions.clientFolder, _stores[_storeNames[i]].path, _errors, e.message);
      }
    }
    ...
    cacheGraphDeps    = cacheGraph(_urlsGraph);
...
function _buildCode (modules, sort, isLunarisStore, storeJoins, isUnderscoreBeforeVar = false, storeReferences) {
  ...
  for (var i = 0; i < sort.length; i++) {
    ...
    var _moduleVarName = _sanitizePath(sort[i]);
    var _moduleCode    = `
      var ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName } = (function(imports, exports) {
        ${ modules[sort[i]].code }
    ...
    if (isLunarisStore) {
      if (modules[sort[i]].from) {
        _code += `${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['data'] = ${ isUnderscoreBeforeVar ? '_' : '' }${ modules[sort[i]].from }_collection;`;
      }
      else {
        _code += `
        var ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }_collection = lunaris._collection(
          ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['getPrimaryKeyFn'],
          ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['isStoreObject'],
          ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['joins'],
          ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['meta'] ? ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['meta'].aggregateFn : null,
          ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['computedsFn'],
          ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['name'],
          ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['references']
        );

        ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }['data'] = ${ isUnderscoreBeforeVar ? '_' : '' }${ _moduleVarName }_collection;
      `;
      }
    }
...
function _insertDependencies (js, options) {
  ...
  _path      = path.join(_buefy, '..', 'buefy.min.js');
  _addDependency(dependenciesCode, _path);
  ...
  // smoothdnd
  _path    = path.join(__dirname, '..', 'deps', 'smooth-dnd.js');
  _addDependency(dependenciesCode, _path);
