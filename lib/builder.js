...
function buildStores (buildOptions, storeNames, callback) {
  ...
  walkStores(buildOptions.storesFolder, (err, stores) => {
    ...
    for (i = 0; i < _storesPathKey.length; i++) {
      ...
      _joinDescriptor['joinFns']                    = storeMap.getJoinFns(_compilatedStores, _compilation.compilation, _compilation.virtualCompilation, _compilation.meta.joins, _compilation.meta.externalAggregates);
...
function _buildCode (modules, sort, isLunarisStore, storeJoins, storeNames = {}) {
  ...
  for (var i = 0; i < sort.length; i++) {
    ...
    var _moduleCode = `
      ${ _sanitizePath(sort[i]) } = (function(imports, exports) {
        ${ modules[sort[i]].code }
  ...
  buildGlobalComponents(buildOptions, globalComponentsCode => {
    buildModules(buildOptions, (modulesCode, paths) => {
      buildStores(buildOptions, storeNames, storesCode => {
        ...
        _code = vuejsIndex
          .replace(/\{\{__LUNARIS__\}\}/g, buildLunaris({ isProduction : buildOptions.isProduction, STORE_DEPENDENCIES : JSON.stringify(storeDependencies) }))
