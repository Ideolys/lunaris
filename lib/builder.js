...
function buildStores (buildOptions, callback) {
  ...
  walkStores(buildOptions.storesFolder, (err, stores) => {
    ...
    _code = _vars + _buildCode(_graphInfos.flattenedObjects, _graphInfos.order, true, _graphJoins) + _code + _codeAfter;
    callback(_code);
  });
}
 ...
 */
function _buildCode (modules, sort, isLunarisStore, storeJoins) {
  var _code = '';
